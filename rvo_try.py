...
walker_angular_velocity_constraint=2
total_time=0.0
...
class Walker:
    ...
    def next_speed(self,theta):
        ...
        elif((theta-np.arctan2(self.vy,self.vx))>walker_angular_velocity_constraint*settle_time):
...
while walker.position[1]<10:
    ...
    ax.clear()
    ax.grid('on')
    ax.add_patch(plt.Circle((walker.position[0], walker.position[1]), walker.r, color='green', alpha=0.5))
    ax.add_patch(plt.Circle((walker.goal[0], walker.goal[1]), 0.1, color='red', alpha=0.5))
    plt.quiver(walker.position[0], walker.position[1], np.cos(walker.velocity_theta), np.sin(walker.velocity_theta), pivot='middle')
    number=0
    for obj in list:
        ax.add_patch(plt.Circle((obj.position[0],obj.position[1]), obj.r, color='black', alpha=0.5))
        plt.text(obj.position[0]-0.1, obj.position[1]-0.1, number, fontsize=14)

        number+=1

    
    for i in range(len(past_route_x)):
        ax.add_patch(plt.Circle((past_route_x[i],past_route_y[i]), 0.02, color='blue', alpha=0.5))

        
    for i in range(len(comfort_x)):
        ax.add_patch(plt.Circle((-comfort_y[i]+4.5,comfort_x[i]+5), 0.02, color='black', alpha=0.5))
